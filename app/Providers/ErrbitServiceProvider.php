...
class ErrbitServiceProvider extends ServiceProvider
{
    ...
    {
        $this->publishes([
            __DIR__.'/../../config/errbit.php' => config_path('errbit.php'),
        ]);
    }
    ...
    public function register()
    {
        ...
        $this->app->singleton(
            ...
            function ($app) {
                ...
                return Errbit::instance()->configure($options)->start();
            }
        );

        if (! $this->isEnabled()) {
            return 'Warning errbit disabled';
        }
    ...
    {
        if (! $app['config']->get('errbit.user.enabled', true)) {
            return [];
        }
