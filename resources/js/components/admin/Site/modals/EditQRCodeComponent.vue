...
export default ({
    props: ['editQrCodeDialog', 'selectedCode', 'slug'],
    data() {
        return {
            valid: true,
            siteId: null,
    ...
    methods: {
        submit() {
            if (this.$refs.form.validate()) {
                ...
                siteQr.append('_method', 'PUT');

                axios.post(`/api/qrCodes/${this.codeId}`, siteQr)
                    .then(res => {
                        this.close();
                        this.$emit('fetchSiteQrCodes');
                        this.$emit('triggerSnackBar', 'QR Code Edited Successfully');
    ...
    watch: {
        selectedCode: function(propVal) {
            this.locationName = propVal.location;
            this.codeId = propVal.qrId;
        }
    }
